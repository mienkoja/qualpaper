...
```{r read_atus_data1, eval=FALSE, tidy=FALSE}
...
act_type <- c(rep("read", obs), rep("sing", obs), rep("trans", obs), rep("meal", obs))
dat_act <- data.frame(min=min, act_type=act_type)

#all distributions are zero-inflated. 
#look at just the non-zero portions to determine our likely distribution
dat_act <- subset(dat_act, dat_act$min > 0)

#plot all four distributions
#exponential is probably not perfect, but is not unreasonable
ggplot(dat_act, aes(x=min, y=..density..)) +
 geom_histogram() +
 facet_wrap(~ act_type)
```
...
ziexp_prior <- function(x, n){
  #create a vector of people who perform the given activity
  #(i.e. a non-zero (nz) vector)
  x_nz <- x[x>0]
  
  #calculate rate parameter with fitdistr 
  param <- fitdistr(x_nz, "exponential")
  
  #specify a zi sampling function 
  #calculate emperical p for the bern portion
  p <- (length(x)-length(x_nz))/length(x)
  
  #simulate all values using the param estimate from above
  prior <- rep(NA, n)
  for (i in 1:n){
    prior[i] <- ifelse(rbern(n=1, p=p)==1
                       ,0
                       ,rexp(n=1, param$estimate[1]))
  }
  #return results
  return(prior)
}
...
```{r breakout_emp_cats, cache=TRUE, tidy=FALSE}
#select employed
...
```{r create_samp_vectors}
read_prior_emp <- ziexp_prior(atus_read_emp, 100000)
sing_prior_emp <- ziexp_prior(atus_sing_emp, 100000)
trans_prior_emp <- ziexp_prior(atus_trans_emp, 100000)
meal_prior_emp <- ziexp_prior(atus_meal_emp, 100000)

read_prior_unemp <- ziexp_prior(atus_read_unemp, 100000)
sing_prior_unemp <- ziexp_prior(atus_sing_unemp, 100000)
trans_prior_unemp <- ziexp_prior(atus_trans_unemp, 100000)
meal_prior_unemp <- ziexp_prior(atus_meal_unemp, 100000)

read_prior_other <- ziexp_prior(atus_read_other, 100000)
sing_prior_other <- ziexp_prior(atus_sing_other, 100000)
trans_prior_other <- ziexp_prior(atus_trans_other, 100000)
meal_prior_other <- ziexp_prior(atus_meal_other, 100000)
...
```{r create_samp_vectors_inc}
inc_prior <- rweibull(100000, 1.3, 50287)*1.16
```
...
```{r read_atus_data2, eval=FALSE, tidy=FALSE}
# load the roster file
load("atusrost.rda")
...
h_p_se <- (4/160)*sum(((colSums(finwgt_pt[,1]*finwgt_pt[,4:163])/colSums(finwgt_pt[,4:163])/60) - h_p)^2)
h_f_se <- (4/160)*sum(((colSums(finwgt_ft[,1]*finwgt_ft[,4:163])/colSums(finwgt_ft[,4:163])/60) - h_f)^2)

upper_h_p <- h_p + h_p_se*qnorm(.975)
lower_h_p <- h_p - h_p_se*qnorm(.975)

upper_h_f <- h_f + h_f_se*qnorm(.975)
lower_h_f <- h_f - h_f_se*qnorm(.975)
```
...
-------------------------
```{r save_all_atus_dat}
priors <- c("mc_hrs_ft", "mc_hrs_pt", "read_prior_other", "sing_prior_other", "trans_prior_other", "meal_prior_other", "meal_prior_unemp", "trans_prior_unemp", "sing_prior_unemp", "read_prior_unemp", "meal_prior_emp", "trans_prior_emp", "sing_prior_emp", "read_prior_emp", "inc_prior")
