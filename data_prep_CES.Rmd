...
```{r create_stub, cache=TRUE}
#st <- readLines("C:/Users/mienkoja/Dropbox/qp_analysis/CES/2004/docs/Programs 2004/IntStub2004.txt") 
...
# c) where ucc indicates childcare
stubfile <- 
  subset( 
    stubfile , 
    ucc %in% c( "670310" , "340212" , "340211") &
      survey != "T" &
      group == "EXPEND"
  )
...
```{r read_ces_data, cache=TRUE, eval=FALSE}
...
cc_own_se <- tab1[4,11]*.91
cc_oth_se <- tab1[5,11]*.91
cc_ctr_se <- tab1[6,11]*.91

cc <- c("cc_own", "cc_oth", "cc_ctr", "cc_own_se", "cc_oth_se", "cc_ctr_se", "cc_by_inc")
